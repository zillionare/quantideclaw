...
6. 设置系统默认进入图形桌面。
7. 在 guest 内安装并启用 `spice-vdagent`，以支持 UTM/SPICE 场景下的宿主机文本粘贴。
8. 在脚本结束时执行安全清理，尽量缩小基础镜像体积。
...
7. 查询后立即测试：候选列表加载出来后，必须立刻使用用户填写的 OpenRouter API Key 对每个候选模型做一次最小请求测试，并把测试结果直接展示在 GUI 中;一旦有模型通过验证，则允许用户点击选择该模型并点击下一步；如果此时用户直接点击下一步也是允许的，则自动使用第一个通过测试的模型作为最终配置。
...
0. 欢迎界面。这里要显示我们的客服微信（quantfans_99，二维码在 guest/assets/quantfans.png
...
4. 引导用户微信扫码（或者）填写 qqbot 需要的 appId 和 appSecret。允许最终用户只用微信，或者只用 QQ，但必须至少选择其中一项。如果用户既没有扫码，又没有填写 appId 和 appSecret，则不允许点击下一步。
5. 除此之外，默认配置 duckduckgo 为搜索引擎，默认启用 tts，并使用 edge-tts 为语音引擎。
6. 调用openclaw 命令，给用户配置的 weixin/qq 发送一条配置完成，欢迎使用的消息。
